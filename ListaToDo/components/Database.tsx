...
export const getDBConnection = async (): Promise<Database> => {
    try{
        return SQLite.openDatabase(dbParms);
    }
    catch(error){
        console.error(error);
...
export const addTodoItem = async (dbb: Database, tarefa: Todo): Promise<void> => {
        try {
            ...
            await dbb.executeSql(sql, [tarefa.id, tarefa.text, 0]);
        } catch (error) {
            console.error(error);
...
export const getTodoItems = async (dbb: Database): Promise <Todo[]> => {
    let returnRows = [] as Todo[];

    try{
        const sql = 'SELECT id, text, done FROM todo_items';
        const rows = await dbb.executeSql(sql);
...
export const updateTodoItem = async (dbb: Database, tarefa: Todo): Promise <void> => {
    ...
    try{
        ...
        await dbb.executeSql(sql, [tarefa.text, 0, tarefa.id]);
    }catch (error){
        console.error(error);
